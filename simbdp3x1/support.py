...
"""Simulation Buddhism Prototype No.3 x.1 - Support
...
from simbdp3x1.common import np_clip, np_random_choice, Child, Dissolution
...
def update_support_aged (economy):
    ...
    for p in economy.people.values():
        ...
        for x in l:
            if x == '' or not economy.is_living(x):
                continue
            c = economy.people[x]
            if c.in_priesthood() or c.in_jail():
                continue
            if c.supported is None:
                ...
                if s.supported is None and s.age >= 18 and s.age < 70 \
                   and not s.in_priesthood() and not s.in_jail() \
    ...
    for p in economy.people.values():
        if p.is_dead() or p.in_jail() \
           or p.supported is not None or p.age >= 70:
            continue
        if p.in_priesthood():
            continue
...
def update_support_aged_priests (economy):
    n_s = 0
    unsup = []
    for p in economy.people.values():
        if p.is_dead() or p.in_jail():
            continue
        if not p.in_priesthood():
            continue
        if p.age < 70 or p.supported is not None:
            continue
        if p.age < 90:
    ...
    print("Support Aged Priests:", n_s)
...
def update_support_infant (economy):
    ...
    need = []
    for p in economy.people.values():
        if p.is_dead() or p.supported is not None \
           or p.age >= 15 or p.married:
            continue
        need.append(p)
    guard1 = [p for p in economy.people.values() if not p.is_dead()
              and p.supported is None and not p.in_jail()]
    guard2 = [p for p in guard1 if p.marriage is not None or p.in_priesthood()]
    ...
    if len(guard) < len(need):
        ...
        g = sorted(g, key=lambda x: int(in_priesthood()))
...
def update_support_unwanted (economy):
    ...
    guard = []
    for p in economy.people.values():
        if p.is_dead() or p.supported is not None or not p.children:
            continue
    ...
    for p in economy.people.values():
        if p.is_dead() or p.in_jail() or p.supported is not None \
           or (p.marriage is None and not p.in_priesthood()) or p.age > 60:
            continue
    ...
    guard = sorted(guard, key=lambda x: x.tmp_asset_rank
                   + 0.5 * random.random(), reverse=True)[0:n]
    guard = sorted(guard, key=lambda x: int(x.in_priesthood()))
    unsup = sorted(unsup, key=lambda x: x.tmp_asset_rank, reverse=False)[0:n]
    unsup.reverse()
...
def update_become_adult (economy):
    for p in economy.people.values():
        if p.is_dead() or p.supported is None \
...
def update_support (economy):
    ...
    update_support_aged_priests(economy)

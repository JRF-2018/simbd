#!/usr/bin/python3
__version__ = '0.0.11' # Time-stamp: <2021-10-26T07:32:18Z>
...
from simbdp3.base import ARGS, Person0, EconomyPlot0, Serializable
from simbdp3.common import np_clip, np_random_choice, Child
from simbdp3.random import normal_levy_rand, normal_levy_1,\
    negative_binominal_distribution
from simbdp3.moving import move_freely_some_people, move_some_people
...
def make_families (economy):
    ...
    for f in families.values():
        ...
        lj = [p for p in f.members.values()
              if p.in_priesthood() or p.in_jail()]
        if lj:
            f.dead_prop = sum([np.mean(p.mlog['prop']) for p in lj])
        ...
        if pls >= mns:
            ...
            for p in f.members.values():
                ...
                if p.marriage is not None and p.marriage.spouse in f.members:
                    if f.members[p.marriage.spouse].is_dead():
                        continue
                    if p.sex == 'M':
